...
?> <main><div class="container"><div class="row diagnosticos"><div class="col-sm-10 offset-1 margin-default"><h1 class="title-page">Blog</h1></div></div><!-- row --></div><!-- container --><div class="container default"><div class="row"><div class="col-sm-2"></div><div class="col-sm-4"></div><!--col-sm-4--><div class="col-sm-2 col-lg-4"></div><div class="col-sm-4 col-lg-2"><div class="form-group"><form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST"><select class="form-control categoria" name="categoria" id="categoria"><option>CATEGORIAS</option><option value="0">Todas as Categorias</option> <?php								
		 	...
		 	?> <div class="col-sm-5"><img class="post-img" src="img/blog/<?php echo $dest->img; ?>"></div><!-- col-sm-5--><div class="col-sm-5"><p class="card-title headline-sub"><?php echo $dest->categoria; ?> | <?php if($dest->data_publicacao != null) {$timestamp = strtotime($dest->data_publicacao); echo date('d/m/Y', $timestamp);} else {echo 'Sem data';} ?></p><h5 class="card-title headline"><?php echo $dest->titulo; ?></h5><p class="blog-text"><?php echo nl2br (substr ($dest->descricao, 0, 400)); ?> [...]</p><p><a href="post.php?id=<?php echo $dest->id_blog; ?>" class="blog-link">Saiba Mais</a></p></div><!-- col-sm-5--> <?php } ?> <?php } ?> </div><!-- row--></div><!--container--><div class="container"><div class="row blog"><div class="col-sm-2"></div><div class="col-sm-10"><div class="row"> <?php								
						if ($blog && $blog->num_rows > 0) {
					  	while($post = $blog->fetch_object()) {	
				 	?> <div class="col-sm-4"><img class="blog-img" src="img/blog/<?php echo $post->img; ?>"><p class="card-title headline-sub"><?php echo $post->categoria; ?> | <?php if($post->data_publicacao != null) {$timestamp = strtotime($post->data_publicacao); echo date('d/m/Y', $timestamp);} else {echo 'Sem data';} ?></p><h5 class="card-title headline"><?php echo $post->titulo; ?></h5><p><a href="post.php?id=<?php echo $post->id_blog; ?>" class="blog-link">Saiba Mais</a></p></div><!--col-sm-3--> <?php } ?> <?php } ?> </div><!-- row --></div><!-- col-sm-10--></div><!-- row--><div class="row"><div class="col-sm-2"></div><div class="col-sm-10 border"></div></div><!--container--><!--<div class="container">
		<div class="row">
	...
	</div><!-- container--></div></div></main><script>document.getElementById('categoria').addEventListener('change', function() {
    this.form.submit();
});</script> <?php
