		...
		switch ($_GET['act']) {
			case 'showupd':
				try {
			    	...
			    	$qry_evento = $conn->query("SELECT id_evento, nome,img,descricao,data_hora,ativo FROM tbl_eventos WHERE id_evento = $id") or trigger_error("27005 - " . $conn->error);

					if ($qry_evento && $qry_evento->num_rows > 0) {
						$dados = "";
		    			while($post = $qry_evento->fetch_object()) {
		    				$dados = '{"id" : "' . $post->id_evento . '", "nome" : "' . $post->nome . '","img" : "' . $post->img . '", "descricao" : "'. $post->descricao .'"  ,"data_hora" : "' . $post->data_hora . '","ativo" : "' . $post->ativo . '", "realizado" : "' . $post->realizado . '"}';
		    			}
					...
					if(isset($_POST) && !empty($_POST) && $_POST["nome"]) {
						...
						if(!$isValid) {
							...
							if(isset($_FILES['img'])) {
							    ...
							    chmod("../../img/eventos/" . $new_name, 644);
							}
							...
							$realizado = (isset($_POST["realizado"]) && $_POST["realizado"] == "1" ? "1" : "0");
							

							$qry_evento = "INSERT INTO tbl_eventos (nome, img, descricao, data_hora, ativo, realizado) VALUES ('" . $nome . "', '" . $new_name . "' ,'" . $descricao . "', '". $data_hora ."' ,'" . $ativo . "','" . $realizado . "')";

							if ($conn->query($qry_evento) === TRUE) {
								$conn->commit();
								echo '{"succeed": true}';
							} else {
						        throw new Exception("Erro ao inserir o slide: " . $qry_evento . "<br>" . $conn->error);
							}							
						}
					}
					else {
			...
			try {
				...
				$qrydel_evento = "DELETE FROM tbl_galeria_eventos WHERE id_evento = $id";
				if ($conn->query($qrydel_evento) === TRUE) {
				
					$qrydelevento = "DELETE FROM tbl_eventos WHERE id_evento = $id";
			        ...
			        throw new Exception("Erro ao remover os times do evento: " . $qrydel_evento . "<br>" . $conn->error);
				}
			} catch(Exception $e) {
				$conn->rollback();
